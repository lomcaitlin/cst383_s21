...
# (modify unemp)
# unemp.reindex(index=major.values, copy=False).iloc[:5].index
...
# assume unemp is defined
(unemp < unemp.loc['COMPUTER SCIENCE']).mean()
...
df[['Median', 'P25th', 'P75th']].sort_values(by='Median',ascending=False).iloc[:10]
...
df[['Median', 'P25th', 'P75th']].sort_values(by='Median',ascending=True).iloc[:10]
...
#
df.groupby('Major_category')['Total'].count()
...
(df[df['HighShareWomen'] == True])['HighShareWomen'].count()/(df['HighShareWomen'].count())
...
(df['Low_wage_jobs']/df['Total']).sort_values(ascending=False).iloc[:10]
...
def weighted_mean(x):
    return x['Median'].mean() * x['Major_share'].sum()
(df.groupby('Major_category').apply(weighted_mean))
