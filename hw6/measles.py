...
# Write your code so that it work even if these values are modified.
...
# Using n and p_measles, randomly initialize the has_measles array.
...
#@ 2
# Using has_measles, set the value of num_measles to the number of True values in has_measles.
# (assignment to num_measles)
num_measles = has_measles[True].sum()

# check the answer
print('Problem 2: {}'.format(num_measles))

#@ 3
# Using variable n, initialize the tests_positive array to all False values
...
# Using p_pos_if_measles, has_measles, and num_measles, randomly initialize the values of 
...
# positive for measles or not.
# Using p_pos_if_okay, has_measles, n, and num_measles, randomly 
# initialize the values of tests_positive for people without measles.
# (update tests_positive)
...
# (expression)
((has_measles[tests_positive][has_measles[tests_positive]]).size/(has_measles.size))/((tests_positive.sum())/tests_positive.size)
...
#@ 9
# # Using the has_measles and tests_positive arrays, create a data frame with two columns:
...
def sim_p_measles_if_pos(p_measles, p_pos_if_measles, p_pos_if_okay):
    ...
    tests_positive[has_measles] = np.random.choice([True, False], num_measles, p=[p_pos_if_measles, 1-p_pos_if_measles])
    tests_positive[~has_measles] = np.random.choice([True, False], n-num_measles, p=[p_pos_if_okay, 1-p_pos_if_okay])
    return ((has_measles[tests_positive][has_measles[tests_positive]]).size/(has_measles.size))/((tests_positive.sum())/tests_positive.size)
...
#@ 12
# Take your code that uses Bayes' Law and create a function 
# that will compute the probability that a person has measles given they their
# test is positive.  You function should have this name, and take these
# parameters:
#     p_measles_if_pos(p_measles, p_pos_if_measles, p_pos_if_okay)
# Do not use any global variables in your functions.
# (define a function)
def p_measles_if_pos(p_measles, p_pos_if_measles, p_pos_if_okay):
    p_pos = (p_pos_if_measles * p_measles) + (p_pos_if_okay * (1-p_measles))
    return (p_pos_if_measles*p_measles)/p_pos
