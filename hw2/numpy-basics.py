...
# In answering a problem, you can assume a variable is defined
# ONLY if it says so in the problem description.  If a problem
# is assumed to be defined, your code must not change it.

# Your answer can contain more than one line of code.  Normally, the
# last line of code will be an assignment statement or an expression.
# Read the problem description to see whether the last line should be
# an assignment statement, an expression, or something else.  For
# example, in problem 1, the last line should be an assignment
# statement.  In problem 3, the last line should be an expression.

# In case you're not sure, an expression is something that could
# appear to the right of the '=' in an assignment statement.
# Examples of expressions:
#    x + y
#    np.array([1,2,3])
# Examples of things that aren't expressions:
#    z = x + y
#    print(x)

# None of your answers should contain print statements.
...
#@ 4
# Compute the result of squaring x.
...
# (write an expression)
np.sum(width)/len(width)

#@ 11
# Write an expression that evaluates to True when arrays 'width' and
# 'length' have the same number of elements.
...
# (write an expression)
(length - np.amin(length))/(np.amax(length) - np.amin(length))
...
# assume array 'parts' is defined
# (write an expression)
np.sqrt((parts[1,0] - parts[0,0])**2 + (parts[1,1] - parts[0,1])**2)
...
def most_similar_part(width, length, parts):
    return part_num[np.isclose(distance(width,length,parts), np.amin(distance(width,length,parts)), atol = 0.01)]
