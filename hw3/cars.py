...
df['mpg'].loc[df['mpg'] > 22].size/df['mpg'].size
...
df['mpg'].loc[df['mpg'] > (df['mpg'].max() * 0.8)]
...
# column values.
df['qsec'].loc[df['mpg'] > (df['mpg'].max() * 0.8)]
...
# in the data set.
df['qsec'].loc[df['mpg'] < (df['mpg'].min() * 1.2)]
...
df['hp'].loc[df['vs'] == 0].loc[df['cyl'] == 6].mean()
